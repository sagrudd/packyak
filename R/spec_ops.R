...
SpecOps = R6::R6Class(
    ...
    initialize = function(pkgobj) {
      ...
      if (self$file_exists()) {
        ...
        if ((private$get_version() == private$pkgobj$get_version()) &
          ...
          cli::cli_alert_success(stringr::str_interp("package [${pkgobj$get_pkg_name()}] with version [${pkgobj$get_version_str()}] exists"))
        } else {
          cli::cli_alert(stringr::str_interp("updating [${pkgobj$get_pkg_name()}] package to version [${pkgobj$get_version_str()}]"))
  ...
  private = list(
    ...
    create_spec_from_template = function() {
    ...
    set_package_versions = function() {
      ...
      private$comment_spec(stringr::str_interp("- updated [${private$pkgobj$get_pkg_name()}] package version to [${version}-${release}] by PackYak"))
    },
    ...
    update_source = function() {
      ...
      summary <- stringr::str_interp("PackYak build of R-package [${private$pkgobj$get_pkg_name()}] version [${private$pkgobj$get_version_str()}]")
